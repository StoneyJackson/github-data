...
class DockerTestHelper:
    ...
    def cleanup_containers() -> None:
        ...
        cmd = ["docker", "ps", "-a", "--filter", f"name={DockerTestHelper.CONTAINER_PREFIX}", "-q"]
        result = subprocess.run(cmd, capture_output=True, text=True)
    ...
    def cleanup_images() -> None:
        ...
        subprocess.run(["docker", "rmi", "-f", DockerTestHelper.IMAGE_NAME], capture_output=True)
...
class TestDockerBuild:
    ...
    def test_built_image_has_python_dependencies(self):
        ...
        cmd = ["pdm", "run", "python", "-c", "import github; print('PyGithub installed')"]
        result = DockerTestHelper.run_container(
            image_tag, environment={}, volumes={}, command=cmd
        )
...
class TestDockerRun:
    ...
    def test_container_runs_with_environment_variables(self, docker_image, temp_data_dir):
    ...
    def test_container_creates_data_directory_structure(self, docker_image, temp_data_dir):
        ...
        cmd = ["sh", "-c", "mkdir -p /data/test && echo 'test data' > /data/test/file.txt"]
        result = DockerTestHelper.run_container(
            docker_image, environment=environment, volumes=volumes, command=cmd
        )
        
        assert result.returncode == 0, f"Data directory creation failed: {result.stderr}"
    ...
    def test_container_handles_missing_environment_variables(self, docker_image, temp_data_dir):
        ...
        assert result.returncode != 0, "Container should fail with missing environment variables"
        # Should have meaningful error message
        error_output = result.stderr.lower()
        assert any(word in error_output for word in ["token", "repo", "environment", "missing"])
...
class TestDockerWorkflow:
    ...
    def test_makefile_docker_build_command_works(self):
        ...
        assert verify_result.stdout.strip(), "Docker image should exist after make build"
    ...
    def test_container_handles_volume_permissions(self, docker_image, temp_data_dir):
        ...
        cmd = ["sh", "-c", "echo 'permission test' > /data/backup/test.txt && cat /data/backup/test.txt"]
        result = DockerTestHelper.run_container(
            docker_image, environment=environment, volumes=volumes, command=cmd
        )
    ...
    def test_container_env_var_substitution_in_makefile_commands(self, temp_data_dir):
        ...
        # Build image first
        build_result = subprocess.run(["make", "docker-build"], capture_output=True, text=True)
        ...
        try:
            ...
            assert result.returncode in [0, 1], f"Make command failed unexpectedly: {result.stderr}"
    ...
    def test_container_resource_limits_and_cleanup(self, docker_image, temp_data_dir):
        ...
        cmd = [
            ...
            "--cpus=0.5",     # Limit CPU
            "-e", "DATA_PATH=/data",
            "-v", f"{temp_data_dir}:/data",
            docker_image,
            "sh", "-c", "echo 'Resource test completed'"
        ]
        
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        
        assert result.returncode == 0, f"Resource limited container failed: {result.stderr}"
    ...
    def test_container_exit_codes_and_error_handling(self, docker_image, temp_data_dir):
        """Test that container returns appropriate exit codes for different scenarios."""
        volumes = {temp_data_dir: "/data"}
        
        # Test successful operation (with fake token, will fail auth but code should handle it)
        environment = {
            "GITHUB_TOKEN": "fake_token",
            "GITHUB_REPO": "owner/repo",
            "DATA_PATH": "/data"
...
class TestDockerPerformance:
    ...
    def test_docker_build_speed_is_reasonable(self):
        ...
        try:
            ...
            assert build_time < 300, f"Docker build took too long: {build_time:.2f} seconds"
    ...
    def test_container_startup_time(self, docker_image):
        ...
        assert result.returncode == 0
        assert startup_time < 10, f"Container startup too slow: {startup_time:.2f} seconds"
