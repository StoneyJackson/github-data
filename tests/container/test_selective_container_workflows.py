...
class TestSelectiveContainerWorkflows:
    ...
    def test_image(self, test_image_name):
        ...
        if not result.stdout.strip():
            ...
            if dockerfile_path.exists():
                success = ContainerTestHelper.build_test_image(dockerfile_path, test_image_name)
                if not success:
    ...
    def test_selective_save_environment_variables(self, test_image, temp_data_dir):
        ...
        # For demonstration, we'll check that the container starts and processes the environment
        ...
        # We mainly verify the container processes the selective environment variables correctly
        assert result.returncode in [0, 1], f"Container failed with unexpected return code: {result.returncode}"
        
        # Check that error messages indicate selective processing was attempted
        output = result.stdout + result.stderr
        assert "INCLUDE_ISSUES" in output or "selective" in output.lower() or len(output) > 0
    ...
    def test_error_scenarios_in_container(self, test_image, temp_data_dir):
        ...
        assert result.returncode != 0, "Container should fail with invalid configuration"
        
        # Error output should contain helpful messages
        error_output = result.stderr.lower()
        assert any(keyword in error_output for keyword in [
            "invalid", "error", "operation", "format", "boolean"
    ...
    def test_resource_constraints_selective_operations(self, test_image, temp_data_dir):
        ...
        try:
            ...
            assert result.returncode in [0, 1, 125, 137], f"Unexpected return code under memory constraint: {result.returncode}"
            ...
            if result.returncode in [125, 137]:
                ...
                assert "memory" in result.stderr.lower() or "killed" in result.stderr.lower()
    ...
    def test_concurrent_container_operations(self, test_image, temp_data_dir):
        ...
        # Check return codes
        assert results["op1"].returncode in [0, 1], f"First operation failed: {results['op1'].returncode}"
        assert results["op2"].returncode in [0, 1], f"Second operation failed: {results['op2'].returncode}"
