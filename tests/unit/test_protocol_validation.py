...
from tests.shared.mocks.protocol_validation import (
    ProtocolValidator, BoundaryMockAuditor, validate_boundary_mock, 
...
class TestProtocolValidator:
    ...
    def test_validate_protocol_completeness_with_complete_mock(self, sample_github_data):
    ...
    def test_validate_protocol_completeness_with_incomplete_mock(self):
        ...
        assert len(issues) > 0
        assert details['completeness_percentage'] < 100.0
        assert len(details['missing_methods']) > 0 or len(details['misconfigured_methods']) > 0
...
""")
        ...
        assert 'mock_boundary' in [p.get('variable') for p in result['manual_patterns'] if p.get('variable')]
...
""")
        ...
        assert result['total_patterns'] >= 0  # May have some patterns from the factory call
