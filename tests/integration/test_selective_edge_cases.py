...
class TestSelectiveEdgeCases:
    ...
    def large_github_data(self):
        ...
        for i in range(100, 130):
            prs.append({
                ...
                "closed_at": None if i % 2 == 0 else f"2023-02-{(i % 28) + 1:02d}T12:00:00Z",
                "merged_at": None if i % 2 == 0 else f"2023-02-{(i % 28) + 1:02d}T12:00:00Z",
    ...
    def mock_github_service(self, large_github_data):
        ...
        github_service.get_repository_pull_requests.return_value = large_github_data["pull_requests"]
        github_service.get_all_issue_comments.return_value = large_github_data["comments"]
        github_service.get_all_pull_request_comments.return_value = large_github_data["pr_comments"]
    ...
    def test_empty_set_specification(self, mock_github_service, storage_service, tmp_path):
        ...
        assert "INCLUDE_ISSUES number specification cannot be empty" in str(exc_info.value)

    def test_single_number_specification(self, mock_github_service, storage_service, tmp_path):
        ...
        for comment in saved_comments:
            assert comment["issue_url"] == "https://api.github.com/repos/owner/repo/issues/25"
    ...
    def test_large_range_specification(self, mock_github_service, storage_service, tmp_path):
    ...
    def test_invalid_number_handling(self, mock_github_service, storage_service, tmp_path):
    ...
    def test_mixed_ranges_and_singles(self, mock_github_service, storage_service, tmp_path):
    ...
    def test_partial_repository_coverage(self, mock_github_service, storage_service, tmp_path):
    ...
    def test_restore_missing_issue_numbers_graceful_handling(self, mock_github_service, storage_service, tmp_path):
        ...
        assert mock_github_service.create_issue_comment.call_count == 6  # 2 per issue * 3 issues

        # Verify comments for available PRs were restored  
        assert mock_github_service.create_pull_request_comment.call_count == 2  # 1 per PR * 2 PRs

    def test_memory_efficiency_large_selection(self, mock_github_service, storage_service, tmp_path):
    ...
    def test_extreme_boundary_values(self, mock_github_service, storage_service, tmp_path):
    ...
    def test_comment_coupling_edge_cases(self, mock_github_service, storage_service, tmp_path):
    ...
    def test_validation_comprehensive_error_messages(self):
        ...
        assert "Operation must be one of" in error_message or "numbers must be positive integers" in error_message