...
class TestPerformanceBenchmarks:
    ...
    def large_repository_data(self):
        ...
        for i in range(1, 1001):
            issues.append({
                ...
                "body": f"This is a performance test issue #{i} with substantial body content. " * 10,
                ...
                "closed_at": None if i % 3 != 0 else f"2023-{(i % 12) + 1:02d}-{(i % 28) + 1:02d}T12:00:00Z",
        ...
        for i in range(1000, 1500):
            prs.append({
                ...
                "body": f"This is a performance test PR #{i} with substantial body content. " * 8,
                ...
                "closed_at": None if i % 4 != 0 else f"2023-{((i - 1000) % 12) + 1:02d}-{((i - 1000) % 28) + 1:02d}T12:00:00Z",
                "merged_at": None if i % 4 != 0 else f"2023-{((i - 1000) % 12) + 1:02d}-{((i - 1000) % 28) + 1:02d}T12:00:00Z",
    ...
    def mock_github_service(self, large_repository_data):
        ...
        github_service = Mock()
        github_service.get_repository_labels.return_value = large_repository_data["labels"]
        github_service.get_repository_issues.return_value = large_repository_data["issues"]
        github_service.get_repository_pull_requests.return_value = large_repository_data["pull_requests"]
        github_service.get_all_issue_comments.return_value = large_repository_data["comments"]
        github_service.get_all_pull_request_comments.return_value = large_repository_data["pr_comments"]
    ...
    @pytest.mark.performance
    def test_selective_vs_full_save_performance(self, mock_github_service, storage_service, tmp_path):
        ...
        if full_timer.duration > 0.1:  # Only assert if full operation took meaningful time
            ...
            assert speedup_ratio >= 2.0, f"Expected at least 2x speedup, got {speedup_ratio:.2f}x"

    @pytest.mark.performance  
    def test_memory_usage_selective_operations(self, mock_github_service, storage_service, tmp_path):
        ...
        for config_name, config in [("small", small_config), ("medium", medium_config), ("large", large_config)]:
        ...
        if memories['small'] > 0:
            ...
            assert memory_ratio <= 50, f"Memory usage grew too much: {memory_ratio:.1f}x for 100x data"
    ...
    def test_api_call_optimization(self, storage_service, tmp_path):
        ...
        mock_github_service.get_repository_labels.side_effect = lambda: track_api_call('get_labels') or []
        mock_github_service.get_repository_issues.side_effect = lambda: track_api_call('get_issues') or []
        mock_github_service.get_repository_pull_requests.side_effect = lambda: track_api_call('get_prs') or []
        mock_github_service.get_all_issue_comments.side_effect = lambda: track_api_call('get_comments') or []
        mock_github_service.get_all_pull_request_comments.side_effect = lambda: track_api_call('get_pr_comments') or []
    ...
    def test_large_selection_performance(self, mock_github_service, storage_service, tmp_path):
        ...
        assert timer.duration < 10.0, f"Large range operation took too long: {timer.duration:.3f}s"

    def test_comment_coupling_performance_impact(self, mock_github_service, storage_service, tmp_path):
        ...
        if no_comments_timer.duration > 0:
            overhead = (comments_timer.duration - no_comments_timer.duration) / no_comments_timer.duration * 100
            print(f"Comment overhead: {overhead:.1f}%")
            
            # Comment coupling should not add excessive overhead (< 200% in most cases)
            assert overhead < 200, f"Comment coupling added too much overhead: {overhead:.1f}%"

    @pytest.mark.performance
    def test_restore_performance_selective_vs_full(self, mock_github_service, storage_service, tmp_path):
        ...
        mock_github_service.create_issue.return_value = {"number": 1001, "title": "Test Issue"}
        mock_github_service.create_pull_request.return_value = {"number": 1501, "title": "Test PR"}
        ...
        full_restore_config = ApplicationConfig(
            ...
            label_conflict_strategy="skip",
            include_git_repo=False,
            include_issues={i for i in range(1, 101)},  # 100 issues (subset for testing)
            include_issue_comments=True,
            include_pull_requests={i for i in range(1000, 1050)},  # 50 PRs
            include_pull_request_comments=True,
            include_sub_issues=False,
        ...
        if selective_restore_timer.duration > 0:
            ...
            assert speedup >= 1.5, f"Expected selective restore to be faster, got {speedup:.2f}x speedup"