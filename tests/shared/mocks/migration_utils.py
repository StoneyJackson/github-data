...
class BoundaryMockMigrator:
    ...
    @staticmethod
    def generate_factory_replacement(
        patterns: List[Dict[str, any]], sample_data_var: Optional[str] = None
    ) -> str:
        ...
        for var_name, var_patterns in variables.items():
            ...
            if not is_boundary:
                ...
                is_boundary = any(
                    any(github in method for github in github_methods)
            ...
            if is_boundary:
                ...
                if sample_data_var:
                    factory_call = (
                        f"MockBoundaryFactory.create_auto_configured({sample_data_var})"
                    )
                else:
    ...
    def create_migration_report(file_path: str, patterns: List[Dict[str, any]]) -> str:
        ...
        for var_name, var_patterns in variables.items():
            ...
            creation_patterns = [
                p for p in var_patterns if p["type"] == "mock_creation"
            ]
            if creation_patterns:
                report.append(
                    f"- **Line {creation_patterns[0]['line']}**: Mock creation"
                )

            method_patterns = [
                p
                for p in var_patterns
                if p["type"] in ["manual_return_value", "manual_side_effect"]
            ]
            if method_patterns:
        ...
        sample_data_suggestions = BoundaryMockMigrator.suggest_sample_data_usage(
            patterns
        )
        if sample_data_suggestions:
            ...
            report.append("")
            replacement = BoundaryMockMigrator.generate_factory_replacement(
                patterns, "sample_github_data"
            )
...
class BoundaryMockValidator:
    ...
    @staticmethod
    def validate_mock_against_protocol(
    ...
    def generate_completeness_report(mock_boundary, protocol_class) -> str:
        ...
        """
        is_valid, missing, incorrect = (
            BoundaryMockValidator.validate_mock_against_protocol(
                mock_boundary, protocol_class
            )
        )
