...
class MockBoundaryFactory:
    ...
    def _configure_all_methods(mock_boundary, sample_data=None):
        ...
        mock_boundary.get_repository_pull_requests.return_value = sample_data.get("pull_requests", [])
        mock_boundary.get_all_pull_request_comments.return_value = sample_data.get("pr_comments", [])
        ...
        mock_boundary.get_repository_sub_issues.return_value = sample_data.get("sub_issues", [])
        mock_boundary.get_issue_sub_issues_graphql.return_value = []
        mock_boundary.get_issue_parent.return_value = None
        mock_boundary.get_rate_limit_status.return_value = {"remaining": 5000, "reset": 3600}

        # Create methods (create_*)
        mock_boundary.create_label.return_value = {"id": 999, "name": "test-label", "color": "ffffff"}
        mock_boundary.create_issue.return_value = {"number": 999, "id": 999, "title": "test-issue"}
        mock_boundary.create_issue_comment.return_value = {"id": 888, "body": "test-comment"}
        mock_boundary.create_pull_request.return_value = {"id": 777, "number": 777, "title": "test-pr"}
        mock_boundary.create_pull_request_comment.return_value = {"id": 666, "body": "test-pr-comment"}
        mock_boundary.create_pull_request_review.return_value = {"id": 555, "body": "test-review"}
        mock_boundary.create_pull_request_review_comment.return_value = {"id": 444, "body": "test-review-comment"}
        ...
        mock_boundary.update_label.return_value = {"id": 999, "name": "updated-label", "color": "000000"}
    ...
    def create_for_restore(success_responses=True):
        ...
        mock_boundary.get_repository_labels.return_value = []  # Empty repository for conflict detection
