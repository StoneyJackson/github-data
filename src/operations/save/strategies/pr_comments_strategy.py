...
class PullRequestCommentsSaveStrategy(SaveEntityStrategy):
    ...
    def _filter_pr_comments_by_prs(self, entities: List[Any], saved_prs: List[Any]) -> List[Any]:
        ...
        print(f"Selected {len(filtered_comments)} PR comments from {len(entities)} total (coupling to {len(saved_prs)} saved PRs)")
    ...
    def _comment_matches_pr(self, comment: Any, saved_pr_identifiers: set) -> bool:
        ...
        if hasattr(comment, 'pull_request_url') and comment.pull_request_url in saved_pr_identifiers:
            return True
