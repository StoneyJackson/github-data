...
class JobDependency:
    ...
    def __init__(self, job: DataTypeJob, depends_on: List[DataTypeJob] = None):
        self.job = job
...
class JobOrchestrator:
    ...
    def execute_jobs(self, job_dependencies: List[JobDependency]) -> List[OperationResult]:
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            ...
            while len(completed_jobs) < len(job_dependencies):
                ...
                if not running_futures and not ready_jobs:
                    remaining_jobs = [dep.job for dep in job_dependencies if dep.job not in completed_jobs]
                    for job in remaining_jobs:
                        error_result = OperationResult(
                            success=False,
                            data_type=job.__class__.__name__.replace('Job', '').lower(),
                            error_message="Job could not be executed due to unresolved dependencies"
                        )
                ...
                if running_futures:
                    ...
                    done, pending = wait(running_futures.values(), return_when=FIRST_COMPLETED)
...
class LabelJob(DataTypeJob):
    ...
    def __init__(self, repo_name: str, output_path: str, collect_labels: 'CollectLabelsUseCase', save_labels: 'SaveLabelsUseCase'):
    ...
    def execute(self) -> OperationResult:
        try:
            # Collect labels
            collect_result = self._collect_labels.execute(CollectLabelsRequest(self.repo_name))
...
class IssueJob(DataTypeJob):
    ...
    def __init__(self, repo_name: str, output_path: str, collect_issues: 'CollectIssuesUseCase', save_issues: 'SaveIssuesUseCase'):
        super().__init__(repo_name, output_path)
        self._collect_issues = collect_issues
        self._save_issues = save_issues
        self.collected_issues = None  # Store for SubIssueJob dependency
    
    def execute(self) -> OperationResult:
        try:
            # Collect issues
            collect_result = self._collect_issues.execute(CollectIssuesRequest(self.repo_name))
...
class CommentJob(DataTypeJob):
    ...
    def __init__(self, repo_name: str, output_path: str, collect_comments: 'CollectCommentsUseCase', save_comments: 'SaveCommentsUseCase'):
        super().__init__(repo_name, output_path)
...
class PullRequestJob(DataTypeJob):
    ...
    def __init__(self, repo_name: str, output_path: str, collect_pull_requests: 'CollectPullRequestsUseCase', save_pull_requests: 'SavePullRequestsUseCase'):
        super().__init__(repo_name, output_path)
...
class PRCommentJob(DataTypeJob):
    ...
    def __init__(self, repo_name: str, output_path: str, collect_pr_comments: 'CollectPRCommentsUseCase', save_pr_comments: 'SavePRCommentsUseCase'):
        super().__init__(repo_name, output_path)
...
class SubIssueJob(DataTypeJob):
    ...
    def __init__(self, repo_name: str, output_path: str, collect_sub_issues: 'CollectSubIssuesUseCase', 
                 associate_sub_issues: 'AssociateSubIssuesUseCase', save_sub_issues: 'SaveSubIssuesUseCase', 
                 save_issues: 'SaveIssuesUseCase', issue_job: IssueJob):
        super().__init__(repo_name, output_path)
        ...
        try:
            ...
            if self._issue_job and self._issue_job.collected_issues is not None:
                ...
                self._save_issues.execute(
                    SaveIssuesRequest(association_response.updated_issues, self.output_path)
                )
...
class SaveRepositoryUseCase(UseCase[SaveRepositoryRequest, List[OperationResult]]):
    ...
    def execute(self, request: SaveRepositoryRequest) -> List[OperationResult]:
        ...
        job_dependencies = self._create_jobs(request.repository_name, request.output_path, data_types)
    ...
    def _create_jobs(self, repo_name: str, output_path: str, data_types: List[str]) -> List[JobDependency]:
        ...
        if "labels" in data_types:
            label_job = LabelJob(repo_name, output_path, self._collect_labels, self._save_labels)
            job_dependencies.append(JobDependency(label_job))
        
        if "issues" in data_types:
            issue_job = IssueJob(repo_name, output_path, self._collect_issues, self._save_issues)
            job_dependencies.append(JobDependency(issue_job))
        
        if "comments" in data_types:
            comment_job = CommentJob(repo_name, output_path, self._collect_comments, self._save_comments)
            job_dependencies.append(JobDependency(comment_job))
        
        if "pull_requests" in data_types:
            pr_job = PullRequestJob(repo_name, output_path, self._collect_pull_requests, self._save_pull_requests)
            job_dependencies.append(JobDependency(pr_job))
        
        if "pr_comments" in data_types:
            pr_comment_job = PRCommentJob(repo_name, output_path, self._collect_pr_comments, self._save_pr_comments)
        ...
        if "sub_issues" in data_types:
            ...
            depends_on = [issue_job] if issue_job and "issues" in data_types else []
            sub_issue_job = SubIssueJob(
                repo_name, output_path, 
                self._collect_sub_issues, self._associate_sub_issues, self._save_sub_issues,
                self._save_issues, issue_job
            )
