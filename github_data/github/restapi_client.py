...
class GitHubRestApiClient:
    ...
    def __init__(self, token: str, github_instance: Optional[Github] = None):
        ...
        Args:
            ...
            github_instance: Optional Github instance to reuse (to avoid multiple instances)
        """
        self._github = github_instance if github_instance else Github(auth=Auth.Token(token))
        self._token = token
